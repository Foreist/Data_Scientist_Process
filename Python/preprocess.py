...
def chunk_file(set_name):
  ...
  # 바이너리 파일을 읽는다를 reader에 저장
  reader = open(in_file, "rb")
  chunk = 0
  finished = False
  # finished가 True가 될 때까지 돌아라
  while not finished:
    # chunks_dir = os.path.join(finished_files_dir, "chunked")
    ...
    with open(chunk_fname, 'wb') as writer:
      ...
      for _ in range(CHUNK_SIZE):
        ...
        # len_bytes에 든 게 없다면 break
        if not len_bytes:
          finished = True
          break

        # pack('iif', 5, 34, 34, 34.00)
...
def tokenize_stories(stories_dir, tokenized_stories_dir):
  ...
  # 토큰화를 준비 중이라는 메시지를 띄움.
  print(("Preparing to tokenize %s to %s..." % (stories_dir, tokenized_stories_dir)))
  # stories_dir 하위 디렉토리를 stories에 저장
...
# 두 번째 라인을 \n로 분리해서 for문에 넣어줌
  for line in lines[1].split('\n'):
# 라인이 비어있다면 continue 아래 거를 실행시키지 않음.
    if line =="":
      continue
# 비어 있지 않다면 line 양 옆의 \n 같은 것을 벗기고 소문자로 바꾸고 띄어쓰기로 구분
    line = line.strip().lower().split()
# sent_label에 line의 마지막 줄을 append
    sent_label.append(line[-1])
    article_lines.append(' '.join(line[:-1]))
